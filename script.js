    ...
    .onSnapshot(
      (snapshot) => {
        ...
        if (memories.length === 0) {
          console.log(
            "Tidak ada data di Firebase, menggunakan data default..."
          );
          memories = [...defaultMemories];
        }
        filteredMemories = [...memories];
      ...
      (error) => {
        ...
        initializeGallery();
        showNotification(
          "Gagal terhubung ke Firebase, menggunakan data default",
          "error"
        );
      }
    );
...
function initializeGallery() {
  console.log(
    "Menginisialisasi galeri dengan",
  ...
  const currentItems = filteredMemories.slice(startIndex, endIndex);
  ...
  currentItems.forEach((memory, index) => {
    ...
    // Use the actual index in the filtered array for lightbox
...
function loadMoreItems() {
  ...
  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = Math.min(startIndex + itemsPerPage, filteredMemories.length);
  const nextItems = filteredMemories.slice(startIndex, endIndex);
  ...
  nextItems.forEach((memory, index) => {
    const galleryItem = document.createElement("div");
    galleryItem.className = "gallery-item fade-in";
    galleryItem.style.animationDelay = `${index * 0.1}s`;
...
async function sendToDiscordWebhook(newMemory) {
  const webhookURL =
    "https://discord.com/api/webhooks/1345608708922806283/26UJQhP-XaR1Ue7FnH9bLgOKeeiOB7o4aFF6v4MAGbjTpavaDOYoV3h3FDFocm26izEK";
...
async function handleAddMemory(e) {
  ...
  if (typeof firebase !== "undefined" && db) {
    try {
      ...
      // Reset to page 1 to show the new memory
      currentPage = 1;
      initializeGallery();
    ...
    showNotification(
      "Gagal menyimpan kenangan, Firebase tidak dimuat",
      "error"
    );
    submitButton.disabled = false;
    submitButton.textContent = originalButtonText;
